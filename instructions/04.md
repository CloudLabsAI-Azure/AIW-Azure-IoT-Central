...
In this exercise, you will see the openness and integration capabilities of IoT Central. As an application platform, IoT Central lets you transform your IoT data into the business insights that drive actionable outcomes. Rules, data export, and the public REST API are examples of how you can integrate IoT Central with line-of-business applications.
...
1. Now open a new tab and Sign in to **Azure portal** at  https://<span></span>portal.azure.com/. When prompted enter the credentials provided in the environment details page.
...
1. In the resource group tab select **iotcentrallogicapp(DeploymentID)** logic app.

1. Now on the logic app page, from the left navigation menu under **Development Tools** click on **Logic app designer**.

1. In **iotcentrallogicapp(DeploymentID) | Logic app designer** go to templates, select **Blank Logic App**.
...
1. On the **When a rule is fired(Preview)** tab, select **Contoso-Connected logistics-DeploymentID** as **Application** and **Temperature-Sensor-Alert** as **Rule**. Click on **+New step**.
...
1. In the **Execute a device command(preview)** page, select the following:
     ...
     - **Application**: Select **Contoso-Connected logistics-DeploymentID**.
 
     - **Device**: Enter the device id, which you copied in the previous task.
     
     - **Device Template**: Select **Logistic Gateway**.
     
       >**Note**: Please select the device template first and then select the other options below.
     
     - **Device Components**: Select **Gateway Commands**.
     
     - **Device Command**: Select **Disable Sensor**.
     
     - **body**: Type **{"Disable Sensor": "DisableSensor"}**.
...
1. In the **Execute a device command(preview)** page, select the following:
     ...
     - **body**: Type **{"Enable Sensor": "EnableSensor"}**.
...
#### Explore connectors for continuous data export of IoT data from IoT Central Application to different destinations such as Azure Event Hubs, Blob Storage. 
 ...
 1. Select **iotcentralsg(DeploymentID)** storage account. In the storage account page from the left navigation menu click on **Access Keys**.
 ...
 1. Click on **+ New export** on the **Data Export** page.
